...
* 作者：微信 r_stata
* 注意：运行代码前请记得把工作目录设置为 cv-all_db.dta 文件所在的文件夹。
use "cv-all_db", clear 
gen value = int(runiform(1, 84))
spmap value using cv-all_coord, id(ID) ///
      ...
      label(data(cv-all_db) x(x_centroid) y(y_centroid) l(name) color(gs10) size(*0.8 ...)) ///
      line(data(cv-all_coord) size(*0.2 ...) color(black))
gr export "cv-all.png", replace
