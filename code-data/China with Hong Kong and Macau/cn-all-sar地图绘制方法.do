...
* 作者：微信 r_stata
* 注意：运行代码前请记得把工作目录设置为 cn-all-sar_db.dta 文件所在的文件夹。
use "cn-all-sar_db", clear 
gen value = int(runiform(1, 84))
spmap value using cn-all-sar_coord, id(ID) ///
      ...
      label(data(cn-all-sar_db) x(x_centroid) y(y_centroid) l(name) color(gs10) size(*0.8 ...)) ///
      line(data(cn-all-sar_coord) size(*0.2 ...) color(black))
gr export "cn-all-sar.png", replace
