...
* 作者：微信 r_stata
* 注意：运行代码前请记得把工作目录设置为 cr-all_db.dta 文件所在的文件夹。
use "cr-all_db", clear 
gen value = int(runiform(1, 84))
spmap value using cr-all_coord, id(ID) ///
      ...
      label(data(cr-all_db) x(x_centroid) y(y_centroid) l(name) color(gs10) size(*0.8 ...)) ///
      line(data(cr-all_coord) size(*0.2 ...) color(black))
gr export "cr-all.png", replace
