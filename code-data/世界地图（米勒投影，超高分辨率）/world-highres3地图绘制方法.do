* world-highres3 地图绘制方法：
* 欢迎关注微信公众号 RStata 获取更多 Stata 相关的学习资料和资源！
* 作者：微信 r_stata
* 注意：运行代码前请记得把工作目录设置为 world-highres3_db.dta 文件所在的文件夹。
use "world-highres3_db", clear 
gen value = int(runiform(1, 84))
spmap value using world-highres3_coord, id(ID) ///
      ...
      label(data(world-highres3_db) x(x_centroid) y(y_centroid) l(name) color(gs10) size(*0.8 ...)) ///
      line(data(world-highres3_coord) size(*0.2 ...) color(black))
gr export "world-highres3.png", replace
