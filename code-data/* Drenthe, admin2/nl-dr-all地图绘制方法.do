...
* 作者：微信 r_stata
* 注意：运行代码前请记得把工作目录设置为 nl-dr-all_db.dta 文件所在的文件夹。
use "nl-dr-all_db", clear 
gen value = int(runiform(1, 84))
spmap value using nl-dr-all_coord, id(ID) ///
      ...
      label(data(nl-dr-all_db) x(x_centroid) y(y_centroid) l(name) color(gs10) size(*0.8 ...)) ///
      line(data(nl-dr-all_coord) size(*0.2 ...) color(black))
gr export "nl-dr-all.png", replace
