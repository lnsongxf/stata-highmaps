...
* 作者：微信 r_stata
* 注意：运行代码前请记得把工作目录设置为 gq-all_db.dta 文件所在的文件夹。
use "gq-all_db", clear 
gen value = int(runiform(1, 84))
spmap value using gq-all_coord, id(ID) ///
      ...
      label(data(gq-all_db) x(x_centroid) y(y_centroid) l(name) color(gs10) size(*0.8 ...)) ///
      line(data(gq-all_coord) size(*0.2 ...) color(black))
gr export "gq-all.png", replace
